...
- **Preload** (`preload.js`): exposes `window.sementics` via `contextBridge`. Each method wraps one `ipcRenderer.invoke` channel handled by `ipcMain.handle` in `main.js`: `runStatements` → `duckdb:run`, `dryRun` → `duckdb:dryRun`, `openFile(fileType)` → `dialog:openFile`, `saveFile(fileType)` → `dialog:saveFile` (`fileType` is a key of `FILE_FILTERS`; catalog `path` params carry it), `saveWorkflow` → `workflow:save`, `openWorkflow` → `workflow:open`. The renderer has no Node access; all Node-side work goes through here.
- **Renderer** (`src/`, ESM, Vite root is `src/`, `base: './'` so it works from `file://`):
  ...
  - `data/nodeCatalog.js` is the registry of node kinds (label, description, icon, accent color). Adding a node type = adding an entry here (inputs/outputs just need `reader`/`format` — `pipeline.js` and `lineage.js` branch on `category`, not `kind`); the palette, node rendering, and minimap colors all read from it.
  ...
  - Dry run: `buildStatements(..., { dryRun: true })` turns outputs into `EXPLAIN COPY …`; views are lazy, so nothing reads rows (only `read_csv`/`read_json` sniff/sample the file for schema). `duckdb:dryRun` binds the statements and returns `DESCRIBE` schemas per view; `lib/lineage.js#buildLineage` turns those into column-level lineage via `sqlingo`'s `lineage()` (pass the `DuckDB` dialect class from `sqlingo/duckdb` — the `'duckdb'` string isn't registered). Lineage lives in `App.jsx` state, reaches nodes through `LineageContext`, and is cleared on any graph edit. Self-check: `node src/lib/lineage.check.mjs`.
  - Export/Open: `lib/workflowFile.js` serializes only authored fields (`id`, `position`, `data.{kind,name,config}`, edge `id/source/target`, viewport) to a versioned JSON file and validates it on load; `App.jsx#onOpen` re-adds runtime fields (`type`, `status`, `onRun`, `defaultEdgeOptions`). Self-check: `node src/lib/workflowFile.check.mjs`.
- **Styling**: Tailwind v4 via `@tailwindcss/vite`; design tokens live in the `@theme` block in `src/index.css` (use `bg-card`, `text-muted-foreground`, etc.). ReactFlow default styles are overridden in the same file. Light/dark: `@theme` holds the dark defaults and `:root[data-theme='light']` overrides them; `lib/useTheme.js` sets `data-theme` (OS preference until the user toggles, then `localStorage`). Never hard-code colors — add a token to both blocks. ReactFlow SVG colors (grid dots, edges, minimap) are set as attributes that can't take `var()`, so they're themed via CSS rules in `index.css`.
