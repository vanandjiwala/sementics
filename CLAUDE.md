...
- **Preload** (`preload.js`): exposes `window.sementics` via `contextBridge`. Each method wraps one `ipcRenderer.invoke` channel handled by `ipcMain.handle` in `main.js`: `runStatements` → `duckdb:run`, `dryRun` → `duckdb:dryRun`, `openCsv` → `dialog:openCsv`, `saveCsv` → `dialog:saveCsv`, `saveWorkflow` → `workflow:save`, `openWorkflow` → `workflow:open`. The renderer has no Node access; all Node-side work goes through here.
